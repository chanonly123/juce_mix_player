...
class JuceMixPlayer : private juce::Timer,
...
private:
    ...
    std::unordered_map<std::string, std::shared_ptr<juce::AudioBuffer<float>>>
    repetedBufferCache;
    ...
    juce::ReferenceCountedObjectPtr<juce::AudioDeviceManager::LevelMeter>
    inputLevelMeter;
    ...
    void _loadRepeatedTrack(int block, int blockDuration,
                            juce::AudioBuffer<float> &output, float offset,
                            float repeatInterval,
                            juce::AudioBuffer<float> *track);
    
    void _loadAudioBlockSafe(int block, bool reset,
                             std::function<void()> completion);
    
    /// loads audio block for `blockDuration` and `block` number. Blocks are
    /// chunks of the audio file.
    ...
    _calculateBlockToRead(float block, MixerTrack &track);
    
    void _createWriterForRecorder();
    
    void flushRecordBufferToFile(juce::AudioBuffer<float> &buffer,
                                 int sampleCount);
